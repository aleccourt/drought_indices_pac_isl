...
def _get_spec_years(start_date,
    ...
    # ABC - TESTING END DATE ISSUE
    _logger.info('_get_spec_years: Date Range: %s', str(dates))
...
def _get_months(start_date,
    ...
    # ABC - TESTING END DATE ISSUE
    test = date_range(start_date_obj, end_date_obj, freq='MS').tolist()
    _logger.info('_get_months: End date arg: %s', str(end_date))
    _logger.info('_get_months: End date date-time obj: %s', str(end_date_obj))
    _logger.info('_get_months: Date Range produced: %s', str(test))
